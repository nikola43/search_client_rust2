...
async fn main() {
    ...
    let jito_tip_instruction = transfer(&payer_keypair.pubkey(), &tip_account, tip_lamports);
    ...
    let bundleResult = send_bundle_with_confirmation(
        &txs,
        &signatures,
        &rpc_client,
        &mut client,
    ...
    // check if the bundle was successful
    match bundleResult {
        Ok(_) => {
